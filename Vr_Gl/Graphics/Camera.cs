...
namespace Vr_Gl
{
    public class Camera
    {
        ...
        protected const float m_speed = 0.25f * 0.08f / MathHelper.PiOver4;//0.25f;
        protected const float m_mouseSpeedX = 0.0045f * 0.08f / MathHelper.PiOver4;
        protected const float m_mouseSpeedY = 0.0025f * 0.08f / MathHelper.PiOver4;
        ...
        public Camera(MainWindow game)
        {
            ...
            Projection = Matrix4.CreatePerspectiveFieldOfView(MathHelper.PiOver4, game.Width / (float)game.Height, 0.01f, 100);
            View = CreateLookAt();
        }
        ...
        public Camera(MainWindow game, Vector3 position, Vector3 target)
            : this(game)
        {
            m_position = position;
        ...
        protected virtual void ProcessInput()
        {
            ...
            //if (mouse[MouseButton.Left])
            {
                m_direction = Vector3.Transform(m_direction,
                    Quaternion.FromAxisAngle(m_up, -m_mouseSpeedX*(mouse.X - m_prevMouse.X))
                    );
                ...
                {
                    m_direction = Vector3.Transform(m_direction,
                        Quaternion.FromAxisAngle(Vector3.Cross(m_up, m_direction), angle)
                        );
                }
